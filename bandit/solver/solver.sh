...
solve_level_28(){
	(
	LVL28_PASSWORD=$(get_password 28)
	REPO_PATH="git_repos_private"
	if [[ ! -d "$REPO_PATH" ]]; then
			mkdir "$REPO_PATH"
	fi
	cd "$REPO_PATH"
	if [[ ! -d "lvl28" ]]; then
		GIT_SSH_COMMAND="sshpass -p '$LVL28_PASSWORD' ssh -o StrictHostKeyChecking=no -p 2220" \
		git clone ssh://bandit28-git@bandit.labs.overthewire.org/home/bandit28-git/repo
		mv repo/ lvl28/
	fi
	cd lvl28/.git
	NEW=$(git log | head -n 1 | awk '{print $2}')
	PREV=$(git log | head -n 7 | tail -n 1 | awk '{print $2}')
	git diff "$NEW" "$PREV" | tail -n 2 | cut -d ' ' -f 3
	)
}

solve_level_29(){
	(
	LVL29_PASSWORD=$(get_password 29)
	REPO_PATH="git_repos_private"
	if [[ ! -d "$REPO_PATH" ]]; then
			mkdir "$REPO_PATH"
	fi
	cd "$REPO_PATH"
	if [[ ! -d "lvl29" ]]; then
		GIT_SSH_COMMAND="sshpass -p '$LVL29_PASSWORD' ssh -o StrictHostKeyChecking=no -p 2220" \
		git clone ssh://bandit29-git@bandit.labs.overthewire.org/home/bandit29-git/repo
		mv repo/ lvl29/
	fi
	cd lvl29/.git
